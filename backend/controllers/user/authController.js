...
// function genVerifyUrl(email) {
//   return `https://${process.env.SITE_NAME}/api/auth/verify?token=${jwt.sign({"email":email, 'action':'verification'}, JWT_SECRET, {expiresIn:'1d'})}`;
// }

// function genSignedUrl (email, action, KEY) {
//   return `http://${process.env.SITE_NAME}/api/auth/${action}?token=${jwt.sign({"email":email}, KEY, {expiresIn:'1d'})}`;
// }
...
const register = async (req, res) => {
  try {
    ...
    const { first_name, last_name, password, email, address, user_type } = req.body;
    ...
    const query = user_type
      ? 'INSERT INTO users (profile_pic, first_name, last_name, password, email, address, user_type) VALUES (?, ?, ?, ?, ?, ?, ?)'
      : 'INSERT INTO users (profile_pic, first_name, last_name, password, email, address) VALUES (?, ?, ?, ?, ?, ?)';

    const params = user_type
      ? [profilePic, first_name, last_name, hashedPassword, email, address, user_type]
      : [profilePic, first_name, last_name, hashedPassword, email, address];
